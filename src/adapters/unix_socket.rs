...
// TODO: make a better struct than public flush_on_send
...
impl Default for UnixSocketListenConfig {
    fn default() -> Self {
        ...
        Self { path: "/tmp/mio.sock".into(), flush_on_send: false }
    }
}
        ...
        match UnixStream::connect(stream_config.path) {
            Ok(stream) => {
                Ok(ConnectionInfo {
                    remote: Self {
                        stream,
                        flush_on_send: stream_config.flush_on_send,
                    },
    ...
    fn listen_with(config: TransportListen, addr: SocketAddr) -> io::Result<ListeningInfo<Self>> {
        ...
        Ok(ListeningInfo {
            local: Self {
                listener,
                bind_path: path_copy,
                flush_on_send: config.flush_on_send,
            },
        ...
        loop {
            match self.listener.accept() {
                Ok((stream, addr)) => accept_remote(AcceptedType::Remote(
                    ...
                    RemoteResource { stream, flush_on_send: self.flush_on_send },
                )),
